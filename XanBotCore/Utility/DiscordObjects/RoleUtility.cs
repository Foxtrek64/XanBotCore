    ...
    public class RoleUtility
    {
        ...
        public static string GetRoleDiffs(DiscordRole oldRole, DiscordRole newRole)
        {
            ...
            if (oldRole.Name != newRole.Name)
                diffs.AppendLine($"**Name:** {oldRole.Name} => {newRole.Name}");
            if (oldRole.Color.Value != newRole.Color.Value)
                diffs.AppendLine($"**Color:** {oldRole.Color.ToString()} => {newRole.Color.ToString()}");
            if (oldRole.IsHoisted != newRole.IsHoisted)
                diffs.AppendLine($"**Display Role Separately:** {oldRole.IsHoisted} => {newRole.IsHoisted}");
            if (oldRole.IsMentionable != newRole.IsMentionable)
                diffs.AppendLine($"**Mentionable:** {oldRole.IsMentionable} => {newRole.IsMentionable}");
            if (oldRole.Position != newRole.Position)
                diffs.AppendLine($"**Position:** {oldRole.Position} => {newRole.Position}");
