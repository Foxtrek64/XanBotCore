...
namespace XanBotCore.Utility.DiscordObjects
{
    ...
    public class ChannelConnectionEventContainer
    {
        ...
        {
            if (XanBotCoreSystem.VoiceClient == null)
                throw new InvalidOperationException("Cannot use connection bindings if the bot can't recognize voice channels in the first place.");
            if (channel.Type != ChannelType.Voice)
                throw new ArgumentException("Expected a voice channel.", "channel");

            if (BindingCache.TryGetValue(channel, out ChannelConnectionEventContainer retn))
                return retn;
