...
namespace XanBotCore.Logging
{
    ...
    public class XanBotLogger
    {
        ...
        public static string LogContainerFolder
        {
            ...
            {
                if (IsPathLocked)
                    throw new InvalidOperationException("Cannot set the log file path after calling XanBotCoreSystem initialize method.");
        ...
        public static ConsoleColorVT DefaultColor
        {
            ...
            set
            {
                ...
                {
                    if (!IsVTEnabled)
                        throw new NotSupportedException("Cannot set DefaultColor to ConsoleColorVT -- VT is not enabled. Did you remember to call EnableVTSupport()?");
                }
        ...
        public static ConsoleColorVT ForegroundColor
        {
            ...
            set
            {
                ...
                {
                    if (value == null)
                        return; // This is safe if it's not enabled since this implies no changes.
        ...
        internal static void WriteMessageFromColors(string message, bool skipConsoleStuff)
        {
            ...
            foreach (string coloredString in colorSegs)
            {
                if (coloredString.Length > 1)
                {
                    byte code = 255;
                    try
                    { code = Convert.ToByte(coloredString.First().ToString(), 16); }
                    catch (Exception) { }
        ...
        {
            if (isDebugModeOnly && (!XanBotCoreSystem.IsDebugMode && !ShowDebugMessagesAnyway))
                return;
