...
namespace XanBotCore.DataPersistence
{
    ...
    public class XConfiguration
    {
        ...
        protected void SetConfigurationValue(string key, string oldValue, string newValue, bool wasJustCreated)
        {
            ...
            {
                try
                { OnConfigValueChanged(BaseHandler.Context, key, oldValue, newValue, wasJustCreated); }
                catch (Exception) { }
            }
            else
            {
                try
                { OnConfigValueChanged(null, key, oldValue, newValue, wasJustCreated); }
                catch (Exception) { }
            }
        ...
        {
            if (cfgFileName == null || cfgFileName == DEFAULT_CFG_FILE_NAME)
